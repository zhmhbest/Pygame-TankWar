...
class Level(object):
    """ 地形图 """
    # 地形常量
    (TILE_EMPTY, TILE_BRICK, TILE_STEEL) = range(3)
    ...
    def draw(self, tiles=None):
        ...
        if tiles == None:
            tiles = [self.TILE_BRICK, self.TILE_STEEL]
    ...
    def updateObstacleRects(self):
        ...
        for tile in self.mapr:
            if tile[0] in (self.TILE_BRICK, self.TILE_STEEL):
...
class Tank(object):
    ...
    def __init__(self, level, side, position=None, direction=None, filename=None):
        ...
        # 坦克出现时的方向
        if direction == None:
            self.direction = random.choice([self.DIR_RIGHT, self.DIR_DOWN, self.DIR_LEFT, self.DIR_UP])
        else:
            self.direction = direction
...
class Enemy(Tank):
    ...
    def __init__(self, level, type, position=None, direction=None, filename=None):
        ...
        # 每两秒发射一颗子弹
        self.timer_uuid_fire = gtimer.add(2000, lambda: self.fire())
    ...
    def spawnBonus(self):
        ...
        # gtimer.add(1000, lambda: bonus.toggleVisibility())
    ...
    def getFreeSpawningPosition(self):
        ...
        available_positions = [
            ...
            [12 * self.level.TILE_SIZE + (self.level.TILE_SIZE * 2 - self.rect.width) / 2,
             (self.level.TILE_SIZE * 2 - self.rect.height) / 2],
            [18 * self.level.TILE_SIZE + (self.level.TILE_SIZE * 2 - self.rect.width) / 2,
             (self.level.TILE_SIZE * 2 - self.rect.height) / 2],
            [24 * self.level.TILE_SIZE + (self.level.TILE_SIZE * 2 - self.rect.width) / 2,
             (self.level.TILE_SIZE * 2 - self.rect.height) / 2],
            [self.level.TILE_SIZE + (self.level.TILE_SIZE * 2 - self.rect.width) / 2,
             (6 * self.level.TILE_SIZE * 2 - self.rect.height) / 2],
            [self.level.TILE_SIZE + (self.level.TILE_SIZE * 2 - self.rect.width) / 2,
             (12 * self.level.TILE_SIZE * 2 - self.rect.height) / 2],
            [self.level.TILE_SIZE + (self.level.TILE_SIZE * 2 - self.rect.width) / 2,
             (18 * self.level.TILE_SIZE * 2 - self.rect.height) / 2],
            [self.level.TILE_SIZE + (self.level.TILE_SIZE * 2 - self.rect.width) / 2,
             (24 * self.level.TILE_SIZE * 2 - self.rect.height) / 2],
            [24 * self.level.TILE_SIZE + (self.level.TILE_SIZE * 2 - self.rect.width) / 2,
             (6 * self.level.TILE_SIZE * 2 - self.rect.height) / 2],
            [24 * self.level.TILE_SIZE + (self.level.TILE_SIZE * 2 - self.rect.width) / 2,
             (12 * self.level.TILE_SIZE * 2 - self.rect.height) / 2],
...
class Game(object):
    ...
    def __init__(self):
        ...
        self.font = pygame.font.Font("heiti.ttf", 16)
    ...
    def triggerBonus(self, bonus, player):
        ...
        labels.append(Label(bonus.rect.topleft, str(trophiesScore), trophiesScore))
    ...
    def gameOver(self):
        """ 游戏结束 """
        global play_sounds, sounds, players
        ...
        # W:记录得分
        Score.writeScore(players[0].score)
        ...
        while main_loop:
            ...
            for event in pygame.event.get():
                ...
                # 选择单人模式还是双人模式的逻辑
                    ...
                    #         self.drawIntroScreen()
                    if event.key == pygame.K_RETURN:  # 按回车键，结束选择
                        main_loop = False
    ...
    # def showScores(self):
    #     """ 计分页面 """
    #     global screen, sprites, players, play_sounds, sounds, highScore
    #
    #     # 终止游戏主循环
    #     self.running = False
    #
    ...
    #
    #     if players[0].score > highScore:
    #         highScore = players[0].score
    #         Score.writeScore(highScore)
    ...
    #     screen.blit(self.font.render(str(highScore), False, pink), [295, 35])
    #
    ...
    def draw(self):
        global screen, castle, players, enemies, bullets, bonuses, highScore
        # 先填充为黑色
        screen.fill([0, 0, 0])
        # 画地形图
        ...
        # 游戏结束了的话，显示"game over"，从基地位置移到屏幕中间，每帧移动8像素
        if self.game_over:
    ...
    def drawSidebar(self):
        """ 画侧边栏 """
        global screen, players, enemies, highScore
        ...
        # W:画当前得分 最高得分（如果当前高于最高那么最高同步当前得分更新） 倒计时
        for n in range(len(self.level.enemies_left) + len(enemies)):
            screen.blit(self.font.render(u"敌人余量-", True, pygame.Color('white')), [xpos, ypos])
            screen.blit(self.font.render(str(len(self.level.enemies_left)), True, pygame.Color('white')),
                        [xpos + 16, ypos + 16])

        if pygame.font.get_init():
            screen.blit(self.font.render(u"最高分-", True, pygame.Color('white')), [xpos, ypos + 60])
            if highScore < players[0].score:
                screen.blit(self.font.render(str(players[0].score), True, pygame.Color('white')),
                            [xpos + 16, ypos + 76])
            else:
                screen.blit(self.font.render(str(highScore), True, pygame.Color('white')), [xpos + 16, ypos + 76])
            screen.blit(self.font.render(u"当前得分", True, pygame.Color('white')), [xpos, ypos + 100])
            screen.blit(self.font.render(str(players[0].score), True, pygame.Color('white')), [xpos + 16, ypos + 116])
    ...
    def drawIntroScreen(self, put_on_surface=True):
        ...
        # W:字母表不知道来源 暂时去掉
        # self.writeInBricks("Tank", [65, 80])
        # self.writeInBricks("War", [129, 160])

        if pygame.font.get_init():
            # 之前获得的最高得分
            highScore = Score.getHighScore()
            # 选择界面的内容
            screen.blit(self.font.render(u"最高得分-" + str(highScore), True, pygame.Color('white')), [170, 55])
            screen.blit(self.font.render(u"开始游戏", True, pygame.Color('white')), [165, 250])
    ...
    def nextLevel(self):
        ...
        # W:只有一关
        levels_enemies = ((60, 30, 20, 10), (60, 30, 20, 10))
                ...
                for player in players:
                    if player.state == player.STATE_ALIVE:
                        if player.bonus != None and player.side == player.SIDE_PLAYER:
                            ...
                            player.bonus = None
                            bonuses.remove(bonus)  # W：移除图像 这里合适吗？ 暂时没发现问题
                    # 命用完就结束，否则产生新的玩家坦克
